      ...
      createRowHtml(row) {
        return '<tr>'
          ...
          + row.map((cell, index) => {
            ...
            if (index === 1) {
              // 会计科目输入框，添加下拉菜单和自动填充功能
              return '<td' + tdClass + '><div class="summary-quick"><input value="' + this.escapeAttr(cell) + '" placeholder="输入科目编码或按F2选择" class="account-code-input"><div class="summary-dropdown" style="position: absolute; top: 100%; left: 0; width: 100%; background-color: white; border: 1px solid #e3edf6; border-radius: 4px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1); z-index: 1000; max-height: 200px; overflow-y: auto; display: none;">' + Object.entries(App.voucher.accountMap).map(([code, name]) => '<div class="summary-item" style="padding: 8px 12px; cursor: pointer; font-size: 12px;">' + code + ' ' + name + '</div>').join('') + '</div></div></td>';
            }
