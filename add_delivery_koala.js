...
async function main() {
  ...
  try {
    ...
    if (users.length === 0) {
      ...
      await conn.execute(
        'INSERT INTO users (user_id, username, phone, user_type, create_time, update_time) VALUES (?, ?, ?, ?, ?, ?)',
        [deliveryData.userId, '考拉', deliveryData.contactPhone, 'enterprise', new Date(), new Date()]
      );
      console.log('✅ 用户创建成功');
    } else {
      console.log('✅ 用户已存在');
